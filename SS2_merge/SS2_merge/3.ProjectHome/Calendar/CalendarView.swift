...
struct CalendarView: View {
        ...
        VStack {
            ...
            ScrollView {
                if !eventsForSelectedDate.isEmpty {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("\(selectedDate, formatter: dateFormatter) - 일정 \(eventsForSelectedDate.count)개")
                            .font(.headline)
                            .padding(.leading, 10)
            ...
            Button(action: {
                ...
                Text("+ 일정 추가하기")
                    ...
                    .cornerRadius(10)
                    .overlay(
                        RoundedRectangle(cornerRadius: 10) // 동일한 cornerRadius 적용
                            .stroke(Color.gray, lineWidth: 0.5) // 테두리 색상과 두께 설정
                    )
                    .padding(.horizontal,10)
            }
            .padding(.bottom, 25)
    ...
    func loadDummyEvents() -> [Event] {
        ...
        let event3 = ("2024-11-19", "1차 코드 git 올리기", "팀 git", Color.orange, ["김현경", "신준용","정광석"])
