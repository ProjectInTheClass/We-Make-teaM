            ...
            db.collection("Submission")
                ...
                .getDocuments { snapshot, error in
                    ...
                    for document in documents {
                        db.collection("Submission").document(document.documentID).updateData([
                            "fileName": uploadedFiles, // uploadedFiles 배열을 fileName 필드에 저장
                            "URL": uploadedTexts, // uploadedTexts 배열을 URL 필드에 저장
                            "isSubmitted" : true
                        ]) { error in
                            if let error = error {
                ...
                Button(action: {
                    if !uploadedFiles.isEmpty || !uploadedTexts.isEmpty {
                        if isSubmitted {
                            ...
                            // 제출 처리
                            updateSubmissionStatus(eventId: event, memberId: currentUserId)
                            onSubmit()
                            isSubmitted = true
                            alertTitle = "제출 완료"
